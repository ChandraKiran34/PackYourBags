...
const PlanTour = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        const response = await axios.get("http://localhost:9000/destinations")
        .then((response) => {
          ...
          <div className="featured-row">
            {filteredProducts.map((place) => (
              <div key={place._id} className="card-hover w-[10vh] p-5">
                ...
                <Link to={`/place/${place._id}/${encodeURIComponent(place.name)}`}>
                <FaHeart className="absolute ml-[21rem] mt-[10px] text-[#fc5252]"/>
                <img className="image ml-5 rounded object-fill h-98 w-96" src={place.picturePath} alt={place.name}  />
                <p className="ml-5 mt-5 font-bold flex"><MdLocationOn className="mt-1"/> {place.location}</p>
